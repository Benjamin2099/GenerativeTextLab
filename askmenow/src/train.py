...
train.py
--------
Trainingsskript (CLI) für die Q&A-Generierung im AskMeNow-Projekt.
Das Skript lädt die vorverarbeiteten Daten (z.B. train.csv und val.csv, die die Spalten 
"question_ids" und "answer_ids" enthalten) und das Vokabular, initialisiert ein Seq2Seq-LSTM-Modell 
(Encoder + Decoder) und trainiert dieses Modell mittels Teacher Forcing.
Beispielaufruf:
...
from dataset import create_qa_dataloader, load_vocab
from model import Encoder, Decoder, Seq2Seq

def parse_args():
    parser = argparse.ArgumentParser(description="Trainiere das Q&A-Modell für AskMeNow")
    # Pfade
    ...
    parser.add_argument("--teacher_forcing_ratio", type=float, default=0.5,
                        help="Wahrscheinlichkeit, das wahre Token als nächstes Input im Decoder zu verwenden")
...
def train_one_epoch(model, loader, optimizer, criterion, device, teacher_forcing_ratio, log_interval):
    model.train()
    total_loss = 0.0
    for batch_idx, (questions, answers) in enumerate(loader):
...
def main():
    ...
    # DataLoader erstellen (das Dataset erwartet die Spalten "question_ids" und "answer_ids")
    train_loader = create_qa_dataloader(args.train_csv, batch_size=args.batch_size, shuffle=True, pad_id=pad_id)
    val_loader = create_qa_dataloader(args.val_csv, batch_size=args.batch_size, shuffle=False, pad_id=pad_id)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            checkpoint_path = os.path.join(args.save_dir, f"askmenow_epoch{epoch}_valloss{val_loss:.4f}.pt")
